...
# --------------------------------------------------------------------------- #
KIT_FIELDS = [
    "História", "Características", "Religião", "Alinhamento", "Custo", "Restrições",
...
def _fix_kit_cost(cost: str) -> str:
    """Repair OCR-dropped leading digit in kit cost.
    'ponto de aprimoramento ...'  -> '1 ponto ...' (singular implies 1).
    'pontos de aprimoramento ...' -> '[?] pontos ...' (plural: 2 or 3, unknown;
    flag for review per the user's 'list pendencies' rule)."""
...
def build_kit(name: str, block: list[str]) -> dict | None:
    ...
    if "Custo" not in fields and "Perícias" not in fields:
        return None
    sub = []
    cost = _fix_kit_cost(fields.get("Custo", "").strip())
    if cost:
        sub.append(make_section("custo", "Custo", "kits", [cost.rstrip(". ")]))
    for label in ("Restrições", "Aprimoramentos", "Perícias", "Armas e Armaduras",
                  "Especial", "Características", "Religião", "Alinhamento", "Aventuras"):
        val = fields.get(label, "").strip()
        if val:
...
def main() -> None:
    ...
    payload = {
        ...
        "status": "pilot_review",
        "summary": "Tormenta (Arton) para o Sistema Daemon: raças, kits/profissões, "
                   "aprimoramentos, especialidades místicas e manobras de combate.",
        "areas": ["racas", "kits", "aprimoramentos", "magias", "manobras_combate"],
        "groups": [],
        ...
        "reviewNotes": [
            ...
            "feitiços. Sistema de magia em si fica como regras (não catalogado aqui).",
            "Group de regras (criação de personagem/atributos/combate) ainda não "
            "incluído neste piloto — pendente.",
        ],
